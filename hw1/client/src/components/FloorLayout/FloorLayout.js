...
const FloorLayout = ({id, cabinets, points, setActiveLayout, setActiveFloor, setActiveCabinet}) => {
        ...
        const renderCircles = cabinets =>
            cabinets.map(cabinet => {
                return <circle className="prefix__st14 circle"
                               onClick={() => onClickCabinet(cabinet.id)}
                               cx={cabinet.pointX - (-10)} cy={cabinet.pointY - 10} r={circleRadius}/>
            });

        const renderCabinets = cabinets =>
            cabinets.map(cabinet => {
                return <g id="prefix__Layer_3">
                    <text
                        transform={"translate(" + (cabinet.pointX - 10 - (cabinet.name.length <= 2 ? 0 : ((cabinet.name.length - 4) * 4) + 5))
                        + " " + (cabinet.pointY - 10) + ")"}
                    ...
                    {!isNaN(parseFloat(cabinet.id)) && isFinite(cabinet.id) &&
                    <text
                        transform={"translate(" + (cabinet.pointX - 10) + " " + (+cabinet.pointY + 10) + ")"}
        ...
        const renderPoints = points =>
            ...
            points.map((point, idx, arr) => {
                ...
                const xPrev = arr[idx - 1].pointX;
                const yPrev = arr[idx - 1].pointY;
                const x = point.pointX;
                const y = point.pointY;
                const floor = point.floor;
                const strokeOpacity = id === floor ? 1.0 : 0.3;
        ...
        const renderStartSign = points =>
            points.length >= 2 &&
            <svg viewBox={"0 0 39 71"}
                 x={points[0].pointX - -9}
                 y={points[0].pointY - 71}
                 width={"39"}
                 height={"71"}
            >
                <path
                    d="M7.068 33.733A18.749 18.749 0 01.696 19.63C.696 9.25 9.111.836 19.49.836c10.38 0 18.794 8.414 18.794 18.794 0 5.618-2.464 10.66-6.371 14.103L19.49 71 7.068 33.733zm12.422-4.706a9.397 9.397 0 100-18.794 9.397 9.397 0 000 18.794z"
                    fill="#ff0000"
                    fillRule="evenodd"
                />
            </svg>;

        const renderFinishSign = points =>
            points.length >= 2 &&
            <svg viewBox="0 0 1000 1000"
                 x={points[points.length - 1].pointX - -10}
                 y={points[points.length - 1].pointY - 71}
                 width={"39"}
                 height={"91"}
            >
                <path
                    d="M217.2 13.75c-22.28 5.9-48.72 19.22-69.69 34.73l-17.48 13.11-2.62-6.33c-3.93-9.61-20.1-23.59-29.49-25.34-21.62-3.93-41.51 4.15-50.9 20.75-9.61 17.04-8.96 21.63 12.45 93.06 10.7 36.04 69.68 233.52 131.07 439.08 61.38 205.34 113.37 377.26 115.34 381.85 1.97 4.59 6.77 11.14 10.7 14.42 21.19 18.57 56.8 12.67 69.25-11.58 9.4-18.13 9.83-16.16-39.76-182.4-25.34-84.54-46.09-155.1-46.09-156.63 0-1.53 11.14-14.2 24.68-27.96 29.28-30.16 44.14-38.46 70.13-40.21 21.63-1.31 42.16 4.15 108.13 29.05 80.61 30.36 146.58 50.46 202.28 61.6 14.86 2.84 30.37 3.71 63.35 3.93 38.01 0 46.09-.65 62.91-4.81 42.38-10.92 73.4-29.27 107.04-63.79 15.73-15.95 21.19-22.94 20.32-25.78-1.75-5.46-51.77-173.01-112.72-378.13-28.18-93.93-51.55-170.61-52.43-170.39-.66.44-12.45 12.67-26.21 27.31-51.55 54.83-75.36 66.84-133.25 66.84-29.71 0-54.83-4.37-95.24-16.16-29.93-8.52-102.02-33.64-140.46-48.93-48.93-19.22-92.62-29.05-138.93-30.8-21.63-.64-29.06-.21-42.38 3.51zm53.3 57.45c1.09 1.53 36.04 116.65 36.04 118.4 0 1.31-6.33 2.4-14.42 2.84-17.91.44-45.66 9.83-62.04 20.54-6.55 4.37-21.85 17.91-33.64 29.93-11.8 12.23-22.06 21.63-22.5 21.19-1.53-1.53-37.79-118.84-37.79-122.11 0-3.71 32.33-34.95 47.19-45.87 13.98-10.05 40.63-22.94 53.52-25.56 7.86-1.54 32.54-1.11 33.64.64zM537 151.37c13.33 3.28 34.3 7.86 46.97 9.83 12.67 2.18 23.16 3.93 23.37 4.15 1.31 1.09 37.57 120.15 36.92 121.02-3.71 3.5-87.81-14.42-130.63-27.96L499 253.82l-9.83-31.02c-5.24-17.04-13.76-44.13-18.79-60.29-5.02-16.17-9.61-30.58-10.27-32.11-.65-1.53 9.61.87 26 6.12 14.85 4.58 37.79 11.36 50.89 14.85zm-178.69 67.94c11.8 3.71 36.26 12.45 54.17 19.44 18.13 6.77 42.16 15.51 53.74 19.44l20.75 6.77 22.94 73.83c14.42 46.53 22.06 73.84 20.53 74.27-2.62.66-28.83-8.52-82.79-29.05-18.13-6.99-41.5-14.64-52.21-17.48l-19.66-5.02-22.72-72.74c-12.23-39.98-22.5-73.62-22.5-74.49.01-3.05 5.91-1.96 27.75 5.03zm469.67 113.15l22.5 74.93-5.02 4.37c-31.02 27.09-80.39 44.78-119.27 42.81l-13.98-.65-22.72-73.83c-12.67-40.63-22.28-74.49-21.63-75.15.66-.66 10.7-2.18 22.06-3.06 37.35-3.06 76.02-17.26 100.49-36.7 6.12-4.59 12.01-8.3 13.11-8.08 1.3.22 12.22 34.3 24.46 75.36zm-463.33 42.6c.22 0 8.3 26.21 18.13 58.11 9.83 31.89 18.35 59.42 18.79 61.38.88 2.84-1.96 3.93-16.38 5.9-38.67 5.46-62.91 17.91-95.02 48.93l-21.84 21.19-18.15-58.32c-9.83-32.11-17.91-59.86-17.91-61.6 0-1.75 11.58-15.29 25.78-29.71 22.5-23.37 27.74-27.52 44.13-35.61 20.1-9.61 36.92-13.54 52.43-11.8 5.45.66 10.04 1.31 10.04 1.53zm224.56 72.31c31.02 8.74 74.49 17.69 96.77 19.88l15.95 1.53 5.68 18.79c3.06 10.27 11.58 37.35 18.57 59.85 6.99 22.72 12.89 42.38 12.89 43.91 0 5.9-65.32-7.86-119.05-25.12l-27.09-8.52-9.83-31.24c-5.24-17.04-13.33-42.82-17.91-57.23-4.37-14.42-8.52-27.52-9.18-29.27-.88-2.4-.22-2.84 2.84-1.53 2.18.86 15.94 4.8 30.36 8.95z"/>
            </svg>;
            ...
            return (
                <svg viewBox="210 180 450 950">
                    ...
                    <g id="prefix__Layer_2">
                        ...
                        />
                        {!!cabinets && renderCabinets(cabinets)}
                        {!!points && renderPoints(points)}
                        {!!points && renderStartSign(points)}
                        {!!points && renderFinishSign(points)}
                    </g>
                </svg>
            )
        }
        ...
        if (id === 3) {
            return <svg viewBox="210 180 450 950">
                <style>
                    {
                        ".prefix__st1,.prefix__st2{fill:none;stroke-miterlimit:10}.prefix__st1{stroke:#000;stroke-width:.12;stroke-linecap:round;stroke-linejoin:round}.prefix__st2{stroke:#939393;stroke-width:.5}.prefix__st3{fill:#939393}.prefix__st4,.prefix__st5{fill:none;stroke:#939393;stroke-width:.75;stroke-miterlimit:10}.prefix__st5{stroke:#000;stroke-width:1.5}.prefix__st6{font-family:&apos;Roboto-Black&apos;}.prefix__st7{font-size:13.0625px}.prefix__st11{fill:#939393;stroke:#939393;stroke-width:.5;stroke-miterlimit:10}.prefix__st14{fill:#add8e6}"
                    }
                </style>
                <path
